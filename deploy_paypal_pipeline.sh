...
echo "PayPal Pipeline complete deployment"
echo "=================================="
echo "This will deploy the entire pipeline from scratch, ensuring perfect operation."
echo ""
...
# check whether is in correct log
check_directory() {
    log_info "Check the directory structure of the items..."

    if ! test -f "docker-compose.yml"; then
        log_error "docker-compose.yml not found - Please run this script in the root directory of the project."
        exit 1
    fi

    if ! test -f ".env"; then
        log_error "didn't find .env file"
        exit 1
    fi

    if ! test -f "sa-key.json"; then
        log_error "didn't find sa-key.json file"
        exit 1
    fi
...
load_environment() {
    ...
    log_info "PROJECTID: $GCP_PROJECT_ID"
    log_info "BUCKET: $GCS_BUCKET"
    log_info "ENVIRONMENT: $ENVIRONMENT"
}

# fix_script_imports problem
fix_script_imports() {
    log_info "Fixed Python script import issue..."

    # fix fetch_transactions.py
    if [[ -f "scripts/fetch_transactions.py" ]]; then
        sed -i 's/from \.utils import/from utils import/g' scripts/fetch_transactions.py
        sed -i 's/from \. import utils/import utils/g' scripts/fetch_transactions.py
        log_success "fix fetch_transactions.py import"
    fi

    # fix transform.py
    if [[ -f "scripts/transform.py" ]]; then
        sed -i 's/from \.utils import/from utils import/g' scripts/transform.py
        sed -i 's/from \. import utils/import utils/g' scripts/transform.py
        log_success "fix transform.py import"
    fi

    # fix load_to_bq.py
    if [[ -f "scripts/load_to_bq.py" ]]; then
        sed -i 's/from \.utils import/from utils import/g' scripts/load_to_bq.py
        sed -i 's/from \. import utils/import utils/g' scripts/load_to_bq.py
        log_success "fix load_to_bq.py import"
    fi

    log_success "All script import issues have been fixed."
}

# update DAG file
update_dag_file() {
    log_info "Update the DAG file to the final version..."

    # Back up the original DAG
    if [[ -f "dags/paypal_dag.py" ]]; then
        cp dags/paypal_dag.py dags/paypal_dag.py.backup.$(date +%Y%m%d_%H%M%S)
        log_info "The original DAG has been backed up."
    fi

    # Create the final version of the DAG
    cat > dags/paypal_dag.py << 'EOF'
# dags/paypal_dag.py
"""
...
EOF
    ...
    log_success "The DAG file has been updated to the final version."
}

# Stop the existing service
stop_services() {
    log_info "Stop the existing Airflow service..."
...
# start service
start_services() {
    log_info "Start the Airflow service..."
    ...
    if docker-compose ps | grep -q "Up"; then
        log_success "Airflow service started successfully"
    else
        log_error "Airflow failed to start"
        docker-compose ps
        exit 1
    fi
}
...
# Verify DAG loading
verify_dag() {
    log_info "verify DAG loading status..."

    # waiting for DAG loading
    sleep 30

    # check DAG list
    if docker-compose exec -T airflow-webserver airflow dags list | grep -q "paypal_data_pipeline"; then
        log_success "PayPal DAG load successfully"
    else
        log_error "DAG load failed"
        docker-compose exec -T airflow-webserver airflow dags list-import-errors
        exit 1
    fi

    # Check import errors
    IMPORT_ERRORS=$(docker-compose exec -T airflow-webserver airflow dags list-import-errors)
    if [[ -n "$IMPORT_ERRORS" ]]; then
        log_warning "DAG import error detected:"
        echo "$IMPORT_ERRORS"
    else
        log_success "NO DAG import error detected"
    fi
}

# Testing DAG operation
test_dag_run() {
    log_info "Testing DAG operation..."

    # trigger DAG operation
    RUN_ID="manual_test_$(date +%Y%m%d_%H%M%S)"

    docker-compose exec -T airflow-webserver airflow dags trigger paypal_data_pipeline --run-id "$RUN_ID"

    log_success "DAG operation triggered: $RUN_ID"
    log_info "Monitor the running status in the Airflow UI.: http://localhost:8080"
    log_info "username: airflow, password: airflow"
}

# show final status
show_final_status() {
    echo ""
    echo " PayPal Pipeline deployment succeed!"
    echo "=================================="
    echo ""
    echo " service status:"
    docker-compose ps
    ...
    echo "   2. check 'paypal_data_pipeline' DAG"
    echo "   3. Manually trigger test execution"
    echo "   4. Check the data in BigQuery"
    echo ""
    echo "PayPal Pipeline deployment succeed!"
}
...
# main function
main() {
    echo "Start the full deployment process..."
    echo ""
    ...
    echo ""
    echo " Congratulations! The PayPal data pipeline has been successfully deployed and is now up and running!"
}
